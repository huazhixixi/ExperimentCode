...
cma_core_type = (complex128[:,:],complex128[:,:],complex128[:,:],complex128[:,:],complex128[:,:],complex128[:,:],double)
...
def equalizer(signal,os,ntaps,mu,iter_number,method = 'cma',mode='training',training_time = 1,train_symbol = None):
    ...
    if method == 'cma':
        ...
        if mode == 'training':
            ...
            for i in range(iter_number):
                if training_time:
                    wxx, wxy, wyx, wyy, error_x, error_y = lms_equalize_core(ex, ey, wxx, wyy, wxy, wyx, mu,1,train_symbol[0],train_symbol[1],np.unique(train_symbol.flatten()))
                    training_time-=1
                else:
                    wxx, wxy, wyx, wyy, error_x, error_y = lms_equalize_core(ex, ey, wxx, wyy, wxy, wyx, mu,training_time,train_symbol[0],train_symbol[1],np.unique(train_symbol.flatten()))
    else:
...
def lms_equalize_core(ex, ey, wxx, wyy, wxy, wyx, mu,is_train,train_symbol_xpol,train_symbol_ypol,constl):
    ...
    for idx in range(len(ex)):
        ...
        error_xpol_array[0, idx] = np.abs(error_ypol)
